...
Route::get('/rebuild-tables', function () {
    try {
        // Drop et recréer trades et trading_accounts proprement
        DB::statement('DROP TABLE IF EXISTS trades CASCADE');
        DB::statement('DROP TABLE IF EXISTS trading_accounts CASCADE');
        
        Schema::create('trading_accounts', function ($table) {
            $table->id();
        ...
        return response()->json(['message' => 'Tables reconstruites avec succes !']);
    } catch (\Exception $e) {
        return response()->json(['error' => $e->getMessage()]);
    }
});